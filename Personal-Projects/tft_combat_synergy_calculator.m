...
% Teamfight Tactics
%
% Combat Synergy Calculator
%
% Calculates optimal team synergy given your current champions
%
% Menu Messages
startsize_msg = 'Select initial team size';
sel1a_msg = 'Select champion';
sel2a_msg = ' Select first champion';
sel2b_msg = ' Select second champion';
sel3a_msg = ' Select first champion';
sel3b_msg = ' Select second champion';
sel3c_msg = ' Select third champion';

% Array of champions
cset = {"Ahri"; "Akali"; "Ashe"; "Bard"; "Blitzcrank"; "Briar"; "Camille"; "Cassiopeia"; "Diana"; "Elise"; "Ezreal"; "Fiora"; "Galio"; "Gwen"; "Hecarim"; ...
...
% Select initial team size
starter_teamsize = {1;2;3};
stz = menu(startsize_msg,starter_teamsize);

% Select initial team

switch stz
    case 1 % 1 Champion
        sel1a = menu(sel1a_msg, cset{1:end});
        
        case 2 % 2 Champions
            sel2a = menu(sel2a_msg, cset{1:end});
            sel2b = menu(sel2b_msg, cset{1:end});

            case 3 % 3 Champions
                sel3a = menu(sel3a_msg, cset{1:end});
                sel3b = menu(sel3b_msg, cset{1:end});
                sel3c = menu(sel3c_msg, cset{1:end});
end

% Select minimum number of incomplete traits
inc_msg = 'Select minimum number of incomplete traits';
inc_set = {0,1};
inc_size = menu(inc_msg,inc_set{1:end});
inc_size = inc_size - 1; % Menu selection offset

% Champion selection variable management

% If team size is equal to 1, other selections are 0
if stz == 1
    sel2a = 0;
    sel2b = 0;
    sel3a = 0;
...
% If team size is equal to 2, other selections are 0
if stz == 2
    sel1a = 0;
    sel3a = 0;
    sel3b = 0;
...
% If team size is equal to 3, other selections are 0
if stz == 3
    sel1a = 0;
    sel2a = 0;
    sel2b = 0;
...
% This code will calculate a set of 60 champions
...
% Varus
varus = zeros(1,27);
varus(5) = 1; varus(20) = 1; % Pyro, Blaster
%
% Veigar
veigar = zeros(1,27);
...
while incomplete_traits > inc_size % Change this value to 1 to make the code complete in 1-5 seconds consistently
    % Synergy checking code
    set = cell(1,8);            % Pre-allocate set

    % Generate champion list with selected team
            % Duplicate while loop
        duplicate(2) = 1;
        
        while duplicate(2) < 8
        
        % Assign full team with existing team
        tg = randperm(60,8);      % Generate random set of 8 numbers from 1 to 60
    ...
    % Modify set code to include initial champion selection
    for i = 1:8
      set{1,i} = champion_name(tg(i));      % Generate a set of champions based on the random numbers
    end
%
    inc = zeros(1,27);          % Pre-allocate set
    
    for n = 1:8
        inc = inc + champion_trait(tg(n),:);      % Generate sum of traits
    end
    %
